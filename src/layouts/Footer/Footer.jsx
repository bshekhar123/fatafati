import React from 'react'
import { Link, } from 'react-router-dom';
import pic1 from "../../layouts/imageslay/logo.png"

const Footer = () => {
  const Links = [
    {
      title: "Company",
      links: [
        {
          name: "Home",
          link: "/"
        },
        {
          name: "About Us",
          link: "/about-us"
        },
        {
          name: "Contact Us",
          link: "/contact-us"
        },
        {
          name: "Movies",
          link: "/movies"
        }
      ]
    },
    {
      title: "Top Categories",
      links: [
        {
          name: "Action",
          link: "#"
        },
        {
          name: "Roamntic",
          link: "#"
        },
        {
          name: "Drama",
          link: "#"
        },
        {
          name: "Sci-Fi",
          link: "#"
        }
      ]
    },
    {
      title: "My Account",
      links: [
        {
          name: "Dashboard",
          link: "/dashboard"
        },
        {
          name: "My Favourites",
          link: "/my favourites"
        },
        {
          name: "Profiles",
          link: "/profiles"
        },
        {
          name: "Change Password",
          link: "/change-password"
        }
      ]
    }
  ]
  return (

    <div className="bg-gray-900 py-4 border-t-2 border-black">
      <div className="container  mx-auto px-2">
        <div className=" grid grid-cols-2 md:grid-cols-7 xl:grid-cols-12 gap-5 sm:gap-9 lg:gap-11 xl:gap-7 py-10 justify-between">
          {Links.map((link, index) => (
            (<div key={index} className="col-span-1 md:col-span-2 lg:col-span-3 pb-3.5 sm:pb-0 ">
              <h3 className="text-md lg:leading-7 font-medium mb-4 sm:mb-5 lg:mb-6 pb-0.5">{link.title}</h3>
              <ul className='text-sm flex flex-col space-y-3'>
                {link.links.map((text, index) => (
                  <li key={index} className="flex items-baseline">
                    <Link
                      to={text.link}
                      className="text-gray-500 inline-block w-full hover:text-[#FF0000]">
                      {text.name}
                    </Link>
                  </li>


                ))
                }

              </ul>

            </div>)
          )

          )}
          <div className="pb-3.5 sm:pb-0 col-span-1 md:col-span-2 lg:col-span-3">
            <Link to='/lists'>
              <img src={pic1} alt="logo" className="w-0.5/3 object-contain h-10" />
            </Link>
            <p className="leading-7 text-sm text-gray-500 mt-3">
              <span>
                1858/1 <br />Rajdanga Main Road Acropolis, 18th Floor <br /> Kolkata, WB 700107
              </span>
              <br />
              <span>Tell: 1800 2121 655</span>
              <br/>
              <span>Email: info@fatafati.com</span>
            </p>

          </div>



        </div>
      </div>
    </div>

  )
}

export default Footer;



